// NC3000 hardware machine implementation.
//
// The NC3000 is the faster sibling of the NC2000 (10.24 MHz CPU). It
// boots from a 1MB NOR Flash (32 x 32KB banks) plus a 66MB NAND Flash
// (two planes), with no big ROM dump and no extended RAM. Banks
// 0x00-0x1F select NOR pages and the fixed BIOS page at 0xE000-0xFFFF is
// NOR bank 0. The IO subsystem follows the SPDC1016 register model
// (timers, ports, LCD address, RTC, DSP, NAND controller), but the NAND
// data register is 0x39 (not 0x29) and the NAND control bits in port 4
...
/// NOR page count (32 x 32KB = 1MB).
const NUM_NOR_PAGES: usize = 0x20;
/// NAND main area page count (131072 pages x 528 bytes, two planes).
const NUM_NAND_PAGES: usize = 65536 * 2;
...
mod io_reg {
    ...
    pub const NAND_DATA: usize = 0x39;
...
impl Nc3000Machine {
    ...
    pub fn new(files: &RomFiles) -> Result<Self> {
        let mut machine = Self {
            ...
            // The NAND image covers two planes plus the 64-page first
            // plane (nand0) at the front of the array, like the NC2000.
            nand: vec![0xFF; (NUM_NAND_PAGES + NAND0_PAGES) * NAND_PAGE_SIZE],
    ...
    fn load_rom_impl(&mut self, files: &RomFiles) -> Result<()> {
        // NOR is loaded in physical order (no bank swap).
        if let Some(nor_path) = &files.nor {
            let data = std::fs::read(nor_path)
                .with_context(|| format!("Failed to read NC3000 NOR: {}", nor_path.display()))?;
    ...
    fn map_read(&self, addr: u16) -> Option<MemTarget> {
        ...
        if addr < 0x4000 {
            // 0x2000-0x3FFF: always RAM page 1 (NC3000 has no RAMB).
            let off = (addr & 0x1FFF) as usize;
            return Some(MemTarget::Ram(0x2000 + off));
        }
        if addr < 0xC000 {
            // 0x4000-0xBFFF: bank window
            let bank_idx = self.io[io_reg::BANK_SWITCH];
            let roa = self.io[io_reg::BIOS_BSW] & 0x80 != 0;
            let page = (addr >> 13) as usize; // 2..5
            let page_off = (addr & 0x1FFF) as usize;
            // NC3000 banks 0 and 1 with ROA set map the whole window
            // onto internal RAM pages (0 -> ram00/02/04/06,
            // 1 -> ram08/0A/0C/0E).
            if roa && bank_idx < 2 {
                let base = if bank_idx == 0 { 0x0000 } else { 0x8000 };
                let map = [0x4000usize, 0x6000, 0x0000, 0x2000];
                return Some(MemTarget::Ram(base + map[page - 2] + page_off));
            }
            // Bank 0 without ROA maps 0x4000-0x7FFF onto RAM
            // (0x4000-0x5FFF and 0x6000-0x7FFF are separate pages).
            if bank_idx == 0 && !roa {
                match page {
                    2 => return Some(MemTarget::Ram(0x4000 + page_off)),
                    3 => return Some(MemTarget::Ram(0x6000 + page_off)),
                    _ => {}
                }
            }
            let bank_off = match page {
                2 => 0x4000,
                3 => 0x6000,
                4 => 0x0000,
                _ => 0x2000,
        ...
        if addr < 0xE000 {
            ...
            let page_off = (addr & 0x1FFF) as usize;
            if bbs_idx == 1 {
                // NC3000 BBS page 1 is RAM page 3 (0x6000-0x7FFF).
                return Some(MemTarget::Ram(0x6000 + page_off));
            }
        ...
        match self.fp_step {
            ...
            5 => {
                if addr_is_5555 && value == 0x10 {
                    ...
                    // Block erase: 4KB aligned block (NC3000).
    ...
    fn nand_write(&mut self, value: u8) {
        // NC3000 NAND control bits in port 4 differ from the NC2000:
        // CLE = bit5, ALE = bit4, CE = bit2.
        let cle = self.io[io_reg::PORT4] & 0x20 != 0;
        let ale = self.io[io_reg::PORT4] & 0x10 != 0;
    ...
    fn io_read(&mut self, addr: u8) -> u8 {
        match addr as usize {
            ...
            0x1E => {
                // Port 6 extension: the NC3000 reads the upper keypad
                // columns through this register.
                self.update_keypad_registers();
    ...
    fn nand_read_mut(&mut self) -> u8 {
        if self.io[io_reg::PORT4] & 0x04 != 0 {
            return 0xFF;
        }
        self.nand_read()
    }
    ...
    /// Wake the device from standby and reset the CPU.
    fn warm_reset(&mut self, cpu: &mut Cpu) {
        self.io[io_reg::TIMER0_VAL] = 1;
        self.io[io_reg::TIMER1_VAL] |= 1;
        self.do_warm_reset = false;
        self.power_off_requested = false;
        cpu.reset(self.peek_u16(crate::cpu::RESET_VECTOR));
        log::info!("NC3000 warm reset (wake from standby)");
    }
...
impl Machine for Nc3000Machine {
    ...
    fn step(&mut self, cpu: &mut Cpu) -> u64 {
        ...
        let frame_cycles = self.cycles_per_frame();
        self.tb_cycles += cycles * TIMER_CYCLES_PER_FRAME;
        let periodic_cycles = self.tb_cycles / frame_cycles;
        self.tb_cycles %= frame_cycles;
        if periodic_cycles != 0 {
            self.tick_periodic(cpu, periodic_cycles as u32);
        }
        cycles
    }
...
/// Bus adapter connecting the shared CPU to NC3000 hardware.
struct Nc3000Bus<'a> {
    machine: &'a mut Nc3000Machine,
}

impl<'a> CpuBus for Nc3000Bus<'a> {
...
mod tests {
    ...
    fn nc3000_rom_files() -> Option<RomFiles> {
        ...
        if nor.exists() && nand.exists() {
            Some(RomFiles::new(None, Some(nor), Some(nand), None))
        } else {
            None
        }
    }
    ...
    fn nor_software_id() {
        ...
        // that is not one of the RAM-mapped special banks.
        m.io_write(io_reg::BIOS_BSW as u8, 0x80);
        m.io_write(io_reg::BANK_SWITCH as u8, 0x02);
    ...
    fn nor_byte_program() {
        ...
        m.io_write(io_reg::BIOS_BSW as u8, 0x80);
        m.io_write(io_reg::BANK_SWITCH as u8, 0x02);
        // Bank 2 window: 0x4000-0x5FFF -> nor[0x10000 + 0x4000].
        assert_eq!(m.nor[0x15234], 0xFF);
        // BYTE_PROGRAM: AA 55 A0 then data
        m.bus_write(0x5555, 0xAA);
        m.bus_write(0xAAAA, 0x55);
        m.bus_write(0x5555, 0xA0);
        m.bus_write(0x5234, 0x5A);
        // CPU 0x5234 maps to NOR offset 0x10000 + 0x4000 + 0x1234
        assert_eq!(m.nor[0x15234], 0x5A);
    }
    ...
    fn nor_block_erase() {
        ...
        m.nor[0x14FFF] = 0x00;
        m.nor[0x15234] = 0x11;
        // BLOCK_OR_MASS_ERASE: AA 55 80 AA 55 30 (4KB block for NC3000)
        m.bus_write(0x5555, 0xAA);
        m.bus_write(0xAAAA, 0x55);
        m.bus_write(0x5555, 0x80);
        m.bus_write(0x5555, 0xAA);
        m.bus_write(0xAAAA, 0x55);
        m.bus_write(0x4234, 0x30); // erase 4KB block at nor[0x14000]
        assert_eq!(m.nor[0x14234], 0xFF);
        assert_eq!(m.nor[0x14FFF], 0xFF);
        assert_eq!(m.nor[0x15234], 0x11); // next block untouched
    }

    #[test]
    fn bank_window_nor_and_ram_specials() {
        let mut m = empty_machine();
        // Bank 0 with ROA set maps the whole window onto RAM pages:
        // 0x4000 -> ram[0x4000], 0x8000 -> ram[0x0000].
        m.io_write(io_reg::BIOS_BSW as u8, 0x80);
        m.io_write(io_reg::BANK_SWITCH as u8, 0x00);
        m.bus_write(0x4000, 0xCD);
        assert_eq!(m.ram[0x4000], 0xCD);
        assert_eq!(m.bus_read(0x4000), 0xCD);

        // Bank 1 with ROA set maps onto ram08/0A/0C/0E.
        m.io_write(io_reg::BANK_SWITCH as u8, 0x01);
        m.bus_write(0x4000, 0x12);
        assert_eq!(m.ram[0xC000], 0x12);
        m.bus_write(0x8000, 0x34);
        assert_eq!(m.ram[0x8000], 0x34);

        // Bank 0 without ROA: 0x4000-0x7FFF is RAM, 0x8000+ is NOR.
        m.io_write(io_reg::BIOS_BSW as u8, 0x00);
        m.io_write(io_reg::BANK_SWITCH as u8, 0x00);
        assert_eq!(m.map_read(0x4000), Some(MemTarget::Ram(0x4000)));
        assert_eq!(m.map_read(0x6000), Some(MemTarget::Ram(0x6000)));
        assert_eq!(m.map_read(0x8000), Some(MemTarget::Nor(0x0000)));
        ...
        // BBS page 1 is RAM page 3 (0x6000).
        m.io_write(io_reg::BIOS_BSW as u8, 0x01);
        assert_eq!(m.map_read(0xC000), Some(MemTarget::Ram(0x6000)));

        // A non-special bank with ROA maps the window onto the NOR.
        m.io_write(io_reg::BIOS_BSW as u8, 0x80);
        m.io_write(io_reg::BANK_SWITCH as u8, 0x02);
        m.nor[0x14000] = 0xAB;
    ...
    fn step_advances_cycles() {
        // Boot with the real NC3000 dump files and verify the CPU advances.
        let Some(files) = nc3000_rom_files() else {
